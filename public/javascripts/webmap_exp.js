...
function refreshLayers(){
    ...
    if(Street_AQI){
        layerControl.removeLayer(Street_AQI);
        map.removeLayer(Street_AQI);
    }
    Street_AQI = L.geoJson(aqi_street, {
        style: line_style,
        // onEachFeature: onEachFeature
    });
    layerControl.addOverlay(Street_AQI, "Street AQI");
    Street_AQI.addTo(map);
...
async function LoadData(){
    ...
    const r2 = await fetch('../qc_streets_interpolated.geojson')
        .then(function (response) {
            console.log(response);
            return response.json();
        })
