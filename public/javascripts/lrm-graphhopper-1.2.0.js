...
(function() {
	...
	L.Routing.GraphHopper = L.Evented.extend({
		...
		_routeDone: function(response, inputWaypoints, callback, context) {
			...
			// 	path = response[i];
			// 	coordinates = this._decodePoints(path.points);
			// 	mappedWaypoints =
			// 		this._mapWaypointIndices(inputWaypoints, path.instruction, coordinates);

			// 	alts.push({
			// 		name: '',
			// 		coordinates: coordinates,
			// 		instructions: this._convertInstructions(path.instruction),
			// 		summary: {
			// 			totalDistance: path.summary.distance,
			// 			mean_aqi: path.summary["mean aqi"],
			// 			totalTime: path.summary.time,
			// 		},
		...
		_decodePoints: function(points){
			var latlngs = new Array(points.length),
				i;
			for (i = 0; i < points.length; i++) {
				latlngs[i] = new L.LatLng(points[i].lat, points[i].lon);
			}
			return latlngs;
		},
		...
		buildRouteUrl: function(waypoints, options) {
			...
			// locs.push('StartLoc=' + waypoints[0].latLng.lng + encodeURIComponent(',') + waypoints[0].latLng.lat);
			// locs.push('EndLoc=' + waypoints[1].latLng.lng + encodeURIComponent(',') + waypoints[1].latLng.lat);

			for (i = 0; i < waypoints.length; i++) {
				locs.push('point=' + waypoints[i].latLng.lng + encodeURIComponent(',') + waypoints[i].latLng.lat);
			}
		...
		_mapWaypointIndices: function(waypoints, instructions, coordinates) {
			...
			for (i = 0; instructions && i < instructions.length; i++) {
				if (instructions[i].sign === 4) { // VIA_REACHED
					idx = instructions[i].interval[0];
					wpIndices.push(idx);
					wps.push({
						latLng: coordinates[idx],
						name: "test" // waypoints[wps.length + 1].name
					});
				}
			}
			...
			wps.push({
				...
				name: "test" // waypoints[coordinates.length - 1].name
			});
